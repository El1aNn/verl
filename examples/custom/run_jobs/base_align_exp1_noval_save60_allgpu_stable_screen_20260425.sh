...
if [[ "${1:-}" != "--foreground" ]]; then
    SESSION_NAME="${SESSION_NAME:-base_align_exp1_noval_save60_allgpu}"
...
"${RAY_CMD}" start --head --node-ip-address="${HOST_IP}" --port 6379 --dashboard-host 0.0.0.0 --dashboard-port 8265 --num-gpus "${GPU_COUNT}" --disable-usage-stats

sleep 2
"${RAY_CMD}" status --address "${RAY_GCS_ADDRESS}"
