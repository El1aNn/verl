...
if [[ "${1:-}" != "--foreground" ]]; then
    SESSION_NAME="${SESSION_NAME:-base_align_exp1_noval_save60}"
...
RAY_CMD="$(command -v ray)"

export SAVE_RUN_SCRIPT=false
export EXP_NAME="${EXP_NAME:-base_align_exp1_noval_save60_$(date +%Y%m%d_%H%M%S)}"
...
"${RAY_CMD}" start --head --port 6379 --dashboard-host 0.0.0.0 --dashboard-port 8265 --num-gpus "${GPU_COUNT}" --disable-usage-stats
