...
if [[ "${1:-}" != "--foreground" ]]; then
    SESSION_NAME="${SESSION_NAME:-base_c_n8_random_wrong}"
...
EXP_NAME="${EXP_NAME:-base_c_n8_random_wrong_2gpu_$(date +%Y%m%d_%H%M%S)}"
MODEL_PATH="${MODEL_PATH:-/root/.cache/modelscope/hub/models/Qwen/Qwen2.5-Math-1.5B}"
TRAIN_FILE="${TRAIN_FILE:-/root/rl/verl/data/dsr_sub/pi1_one_ans_reward_random_wrong.parquet}"
...
METRICS_FILE="${METRICS_FILE:-${CKPTS_DIR}/metrics/verl_grpo_dsr_sub_baseline/${EXP_NAME}.jsonl}"
