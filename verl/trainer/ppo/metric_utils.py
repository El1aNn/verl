...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # For outcome reward, we only care about the reward at the last token.
    valid_indices = (response_length - 1).long().clamp(min=0)
    sequence_reward = batch.batch["token_level_rewards"][torch.arange(batch.batch["token_level_rewards"].shape[0]), valid_indices]
...
def compute_pass_at_k(n, c, k):
    """
    Compute pass@k metric using the combinatorial formula.
...
def _infer_correct_mask_from_values(vals: "np.ndarray") -> "np.ndarray | None":
    """Infer which values should be considered 'correct' for pass@k.

    This function is intentionally conservative: it returns None for continuous metrics
    like reward to avoid producing misleading pass@k values.

    Supported conventions:
    - {0, 1} -> correct if > 0.5
    - {-1, 1} -> correct if > 0
    - [0, 1] probabilities -> correct if > 0.9 (legacy behavior)
    """
        ...
        for uid, var2vals in uid2var2vals.items():
            for var_name, var_vals in var2vals.items():
                ...
                try:
                    ...
                    # Skip values that cannot be treated as numeric scalars
                    continue
