...
\date{Capstone Project Midterm Presentation}
...
Base full-data checkpoints at steps 0, 60, 120, 180; 32 held-out problems; 4 sampled responses per problem; every generated token is scored.
\end{block}
...
\begin{itemize}
    ...
    \item At the final checkpoint, correct responses remain lower-entropy and higher top-1 than wrong responses.
...
% 14
\begin{frame}{Where the Gain Appears in Training}
\begin{figure}
    \centering
    \includegraphics[width=\linewidth,height=0.66\textheight,keepaspectratio]{pic/phase-gain-breakdown.png}
    \caption{Early and late gain decomposition.}
\end{figure}
\small Most of the useful improvement appears early, consistent with activating and stabilizing existing behavior rather than learning a broad new skill from one sample.
\end{frame}

% 15
\begin{frame}{Why Accuracy Alone Is Not Enough}
\begin{figure}
    \centering
    \includegraphics[width=\linewidth,height=0.62\textheight,keepaspectratio]{pic/endpoint-frontier-panel.png}
\end{figure}
\small A good endpoint is not only high score. It should also occupy a stable region of entropy, low-confidence mass, and response length.
\end{frame}

% 16
\begin{frame}{Mechanism Evidence: Score Gain Couples With Calibration}
\begin{figure}
    \centering
    \includegraphics[width=\linewidth,height=0.60\textheight,keepaspectratio]{pic/gain-calibration-coupling.png}
    \caption{Score gain coupled with token-level calibration indicators.}
\end{figure}
\small Score improvement aligns with distributional stabilization across settings.
\end{frame}

% 17
\begin{frame}[shrink=8]{Source of the Gain: Reward Semantics Matter}
\begin{columns}
\column{0.52\textwidth}
\begin{figure}
    \centering
    \includegraphics[width=\linewidth,height=0.46\textheight,keepaspectratio]{pic/actor-entropy-static.png}
\end{figure}
...
The evidence points to behavioral calibration: shorter responses, lower entropy, higher top-1 probability, fewer low-confidence tokens, and better-calibrated correct traces.
\end{block}
\end{frame}

% 19
\begin{frame}{Boundaries of the Claim}
\begin{itemize}
    \setlength{\itemsep}{0.65em}
    \item Current results are single-run evidence for each setting; more seeds are needed.
    \item The random-reward control is only matched for the Base family.
    \item Token diagnostics are richer than score curves, but coverage is still uneven across evidence types.
    \item The experiments focus on Qwen2.5-Math-1.5B and mathematical reasoning.
\end{itemize}
\takeaway{The claim is mechanistic and scoped: strong evidence for calibration in this setting, not a universal statement about all RLVR.}
\end{frame}

% 20
\begin{frame}{Next Steps: Strengthening the Mechanism Claim}
\begin{block}{Near-Term}
\begin{itemize}
    \item Replicate n=1 vs. n=8 and one-shot vs. full-data with multiple seeds.
    \item Stratify held-out problems by difficulty and topic.
    \item Expand full-token checkpoint audits to more settings.
\end{itemize}
\end{block}

\begin{block}{Long-Term}
Scale to larger math models and test whether behavioral calibration transfers to code and scientific QA.
\end{block}
\end{frame}
