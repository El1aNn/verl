...
def _patch_transformers_config_mapping_register_for_vllm() -> None:
    """Avoid import-time crashes from vLLM registering duplicate Transformers config keys.

    Some vLLM versions register extra configs (e.g. Ovis/AIMv2) via
    `transformers.AutoConfig.register(...)` without `exist_ok=True`.
    Newer Transformers may already include that key (e.g. 'aimv2'), causing:
      ValueError: '<key>' is already used by a Transformers config

    This patch makes the registry tolerant by retrying with `exist_ok=True`.
    It is intentionally narrow (only affects duplicate-key errors).
    """

    try:
        from transformers.models.auto import configuration_auto as _configuration_auto
    except Exception:
        return

    mapping = getattr(_configuration_auto, "CONFIG_MAPPING", None)
    if mapping is None:
        return

    if getattr(mapping.register, "__verl_patched_for_vllm__", False):
        return

    original_register = mapping.register

    def _register(key, value, exist_ok=False):
        try:
            return original_register(key, value, exist_ok=exist_ok)
        except ValueError as exc:
            message = str(exc)
            if (not exist_ok) and ("already used by a Transformers config" in message):
                return original_register(key, value, exist_ok=True)
            raise

    _register.__verl_patched_for_vllm__ = True
    mapping.register = _register


_patch_transformers_config_mapping_register_for_vllm()
