...
{}\markdownRendererHeadingThree{中文讲法}\markdownRendererInterblockSeparator
{}这里是 full-token audit。我取 Base full-data 的 step 0、60、120、180，在 held-out problems 上记录每一个生成 token 的指标。\markdownRendererInterblockSeparator
...
{}\markdownRendererHeadingThree{English version}\markdownRendererInterblockSeparator
{}This is the full-token audit. I take Base full-data checkpoints at steps 0, 60, 120, and 180, and record metrics for every generated token on held-out problems.\markdownRendererInterblockSeparator
...
{}\markdownRendererHeadingThree{中文讲法}\markdownRendererInterblockSeparator
{}这一页想说明为什么不能只看 accuracy。两个 checkpoint 的分数可能很接近，但 entropy、low-confidence mass 和 length 其实不一样。\markdownRendererInterblockSeparator
{}所以我认为一个好的 endpoint 不只是分数高，还应该更稳定、不确定性更低。\markdownRendererInterblockSeparator
{}\markdownRendererHeadingThree{English version}\markdownRendererInterblockSeparator
...
{}So I would say a good endpoint should not only score well. It should also be stable and low-uncertainty.\markdownRendererInterblockSeparator
{}\markdownRendererHorizontalRule{}\markdownRendererInterblockSeparator
...
{}\markdownRendererHeadingThree{中文讲法}\markdownRendererInterblockSeparator
{}当然，这个结论也有边界。目前主要还是 single-run evidence；random-reward 只在 Base family 做了匹配；token diagnostics 的覆盖还不是完全均匀；模型也主要集中在 Qwen2.5-Math-1.5B。\markdownRendererInterblockSeparator
{}所以我会把这个结论说得比较有范围：它是一个 scoped mechanistic claim，而不是对所有 RLVR 设置的普遍结论。\markdownRendererInterblockSeparator
{}\markdownRendererHeadingThree{English version}\markdownRendererInterblockSeparator
{}Of course, the claim has boundaries. The current evidence is mainly single-run evidence. The random-reward control is matched only for the Base family. Token diagnostics are not equally complete for every setting. And the model is mainly Qwen2.5-Math-1.5B.\markdownRendererInterblockSeparator
{}So I would frame the conclusion as a scoped mechanistic claim, not a universal statement about all RLVR settings.\markdownRendererInterblockSeparator
{}\markdownRendererHorizontalRule{}\markdownRendererInterblockSeparator
{}\markdownRendererHeadingTwo{20. Next Steps: Strengthening the Mechanism Claim}\markdownRendererInterblockSeparator
{}\markdownRendererHeadingThree{中文讲法}\markdownRendererInterblockSeparator
{}下一步，我会补 multi-seed，尤其是 n=1 vs. n=8 和 one-shot vs. full-data；然后按题目难度和 topic 分层；最后把 full-token audit 扩展到更多设置。\markdownRendererInterblockSeparator
{}更长期一点，我想看这个 calibration framework 能不能扩展到更大模型、code reasoning 和 scientific QA。\markdownRendererInterblockSeparator
{}\markdownRendererHeadingThree{English version}\markdownRendererInterblockSeparator
...
{}Longer term, I want to test whether this calibration framework can extend to larger models, code reasoning, and scientific QA.\markdownRendererInterblockSeparator
{}\markdownRendererHorizontalRule{}\markdownRendererInterblockSeparator
