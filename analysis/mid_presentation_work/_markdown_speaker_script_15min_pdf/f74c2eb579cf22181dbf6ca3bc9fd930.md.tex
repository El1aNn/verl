...
{}对应文件：\markdownRendererCodeSpan{slide.tex} 当前 17 页版本\markdownRendererLineBreak
...
{}\markdownRendererHorizontalRule{}\markdownRendererInterblockSeparator
{}\markdownRendererHeadingTwo{13. Wrong Trace: High Entropy Marks the Failure Point}\markdownRendererInterblockSeparator
{}\markdownRendererHeadingThree{中文讲法}\markdownRendererInterblockSeparator
{}这一页换成一个错误 trace。这个例子没有 Python，也没有很奇怪的 verification，它就是一道正弦函数读图题，所以错误更清楚。\markdownRendererInterblockSeparator
{}模型前面其实做对了：振幅是 2，周期对应 \markdownRendererCodeSpan{b=3}，中线是 \markdownRendererCodeSpan{d=1}。真正出错的是它说 “the graph reaches its maximum at x=0”。这里高 entropy 正好也集中在 \markdownRendererCodeSpan{graph reaches}、\markdownRendererCodeSpan{seems}、phase 这些读图和相位判断附近。\markdownRendererInterblockSeparator
{}但实际函数在 \markdownRendererCodeSpan{x=0} 的值是 \markdownRendererCodeSpan{2 sin(pi)+1=1}，这是中线，不是最大值。模型一旦把这里看成最大值，就会推出 \markdownRendererCodeSpan{sin(c)=1}，最后得到错误的 \markdownRendererCodeSpan{pi/2}。所以这一页想说明：高熵有时会提前暴露“错误前提”在哪里，而不只是告诉我们最终答案错了。\markdownRendererInterblockSeparator
{}\markdownRendererHeadingThree{English version}\markdownRendererInterblockSeparator
{}This slide shows a wrong trace. It is cleaner than the previous example because there is no Python verification. It is just a sine-graph reading problem, so the failure is easier to locate.\markdownRendererInterblockSeparator
{}The model gets several early facts right: the amplitude is 2, the period gives \markdownRendererCodeSpan{b=3}, and the midline gives \markdownRendererCodeSpan{d=1}. The real mistake is the sentence “the graph reaches its maximum at x=0.” The high-entropy words also concentrate around \markdownRendererCodeSpan{graph reaches}, \markdownRendererCodeSpan{seems}, and the phase or graph-reading choices.\markdownRendererInterblockSeparator
{}But in the actual function, at \markdownRendererCodeSpan{x=0}, we have \markdownRendererCodeSpan{2 sin(pi)+1=1}, which is the midline, not the maximum. Once the model treats it as a maximum, it derives \markdownRendererCodeSpan{sin(c)=1} and ends with the wrong answer \markdownRendererCodeSpan{pi/2}. So this trace shows that high entropy can point to the mistaken premise before the final answer appears.\markdownRendererInterblockSeparator
{}\markdownRendererHorizontalRule{}\markdownRendererInterblockSeparator
{}\markdownRendererHeadingTwo{14. Mechanism Interpretation}\markdownRendererInterblockSeparator
{}\markdownRendererHeadingThree{中文讲法}\markdownRendererInterblockSeparator
{}full-token audit 给了几个机制信号：收益不是靠更长输出；分数差不多的 checkpoint 行为也可能不同；正确响应通常低熵、高 top-1；错误 trace 里，高熵也可以帮我们定位错误前提；EOS 的变化也说明这不是简单 early stopping。\markdownRendererInterblockSeparator
{}所以这里的机制不是某一个指标单独在起作用，而是 confidence、entropy、length 和 stopping behavior 一起发生了变化。\markdownRendererInterblockSeparator
{}\markdownRendererHeadingThree{English version}\markdownRendererInterblockSeparator
{}The full-token audit gives several mechanism signals: the gain is not from longer outputs; checkpoints with similar scores can still behave differently; correct responses usually have lower entropy and higher top-1; wrong traces can reveal high-entropy mistaken premises; and EOS changes show that this is not simply early stopping.\markdownRendererInterblockSeparator
...
{}\markdownRendererHorizontalRule{}\markdownRendererInterblockSeparator
{}\markdownRendererHeadingTwo{15. Source of the Gain: Reward Semantics Matter}\markdownRendererInterblockSeparator
{}\markdownRendererHeadingThree{中文讲法}\markdownRendererInterblockSeparator
...
{}\markdownRendererHorizontalRule{}\markdownRendererInterblockSeparator
{}\markdownRendererHeadingTwo{16. Answering the Three Questions}\markdownRendererInterblockSeparator
{}\markdownRendererHeadingThree{中文讲法}\markdownRendererInterblockSeparator
...
{}\markdownRendererHorizontalRule{}\markdownRendererInterblockSeparator
{}\markdownRendererHeadingTwo{17. Thank You}\markdownRendererInterblockSeparator
{}\markdownRendererHeadingThree{中文讲法}\markdownRendererInterblockSeparator
...
{}\markdownRendererHeadingThree{English answer}\markdownRendererInterblockSeparator
{}Do not read the whole trace. Say that this is a real score-1 generation, the background is entropy, it reaches \markdownRendererCodeSpan{\markdownRendererBackslash{}boxed\markdownRendererLeftBrace{}28\markdownRendererRightBrace{}}, and warm colors mainly appear around reasoning choices and transitions.\markdownRendererInterblockSeparator
{}\markdownRendererHeadingTwo{Q6. 错误 trace 这一页怎么讲？ / How should I present the wrong trace?}\markdownRendererInterblockSeparator
{}\markdownRendererHeadingThree{中文回答}\markdownRendererInterblockSeparator
{}重点讲错误发生在 \markdownRendererCodeSpan{graph reaches its maximum at x=0} 这个前提上。高 entropy 也集中在 \markdownRendererCodeSpan{graph reaches} 和 phase 判断附近，所以它不是只在最终答案处才显示不确定，而是在错误推理转折点就已经有信号。\markdownRendererInterblockSeparator
{}\markdownRendererHeadingThree{English answer}\markdownRendererInterblockSeparator
{}Focus on the mistaken premise: \markdownRendererCodeSpan{the graph reaches its maximum at x=0}. The high-entropy tokens cluster around \markdownRendererCodeSpan{graph reaches} and the phase decision, so uncertainty appears at the reasoning turn, not only at the final answer.\markdownRendererDocumentEnd