...
if [[ "${1:-}" != "--foreground" ]]; then
    SESSION_NAME="${SESSION_NAME:-base_c_n1_val8_ray}"
...
EXP_NAME="${EXP_NAME:-base_c_n1_val8_2gpu_ray_$(date +%Y%m%d_%H%M%S)}"
...
METRICS_DIR="${METRICS_DIR:-${CKPTS_DIR}/metrics}"
...
SWANLAB_LOG_DIR="${SWANLAB_LOG_DIR:-/root/rl/verl/swanlog/${EXP_NAME}}"
SWANLAB_MODE="${SWANLAB_MODE:-cloud}"
VERL_FILE_LOGGER_PATH="${VERL_FILE_LOGGER_PATH:-${METRICS_DIR}/verl_grpo_dsr_sub_baseline/${EXP_NAME}.jsonl}"
RESOLVED_RUNTIME_ENV="/tmp/verl_runtime_env_${EXP_NAME}.yaml"

mkdir -p "$(dirname "${LOG_FILE}")" "${CKPTS_DIR}" "${METRICS_DIR}" "${REPORT_DIR}" \
    "$(dirname "${TRITON_CACHE_DIR}")" "$(dirname "${TORCHINDUCTOR_CACHE_DIR}")" \
    "${SWANLAB_LOG_DIR}" "$(dirname "${VERL_FILE_LOGGER_PATH}")"
...
printf '  SWANLAB_LOG_DIR: "%s"\n' "${SWANLAB_LOG_DIR}" >> "${RESOLVED_RUNTIME_ENV}"
printf '  SWANLAB_MODE: "%s"\n' "${SWANLAB_MODE}" >> "${RESOLVED_RUNTIME_ENV}"
printf '  VERL_FILE_LOGGER_PATH: "%s"\n' "${VERL_FILE_LOGGER_PATH}" >> "${RESOLVED_RUNTIME_ENV}"
...
echo "swanlab_log_dir: ${SWANLAB_LOG_DIR}"
echo "file_logger_path: ${VERL_FILE_LOGGER_PATH}"
echo "====================="
